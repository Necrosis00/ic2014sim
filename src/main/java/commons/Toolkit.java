...
public class Toolkit {
	...
	public static double randomWithinRange(Range outputRange) {
		...
		double result = Toolkit.mapValueToOtherRange(randomRange, outputRange,
				value);
		return result;
	}

	// maps the input value from one range to the proportional value in another
	// range
	public static double mapValueToOtherRange(Range inputRange,
			Range outputRange, double value) {
		return (value - inputRange.getMin())
				* (outputRange.getMax() - outputRange.getMin())
				/ (inputRange.getMax() - inputRange.getMin())
				+ outputRange.getMin();
	}
	...
	public static double direction(Position pos1, Position pos2) {
		...
		if (pos1.getX() < pos2.getX() && pos1.getY() < pos2.getY()) {
			// dy / dx, 90 - winkel
			double angle = Math.toDegrees(Math.atan(Math.abs(pos1.getY()
					- pos2.getY())
		...
		if (pos1.getX() < pos2.getX() && pos1.getY() > pos2.getY()) {
			// dx / dy, 180 - winkel
			double angle = Math.toDegrees(Math.atan(Math.abs(pos1.getX()
					- pos2.getX())
